...
class Evaluator {
 public:
  ...
  // Run for each tree.
  virtual void Gather(lczero::classic::NodeTree* tree) = 0;
  // Run once between Gather and Move.
  virtual void Run() = 0;
  // Run for each tree in the same order as Gather.
  virtual void MakeBestMove(lczero::classic::NodeTree* tree) = 0;
};
...
class MultiSelfPlayGames {
 public:
  ...
  std::vector<Move> GetMoves(int index) const {
    ...
    bool flip = !trees_[index]->GetPositionHistory().IsBlackToMove();
 ...
 private:
  ...
  // tree_[0] == tree_[1].
  std::vector<std::shared_ptr<lczero::classic::NodeTree>> trees_;
