...
void ApplyDirichletNoise(Node* node, float eps, double alpha) {
  ...
  for (const auto& child : node->Edges()) {
    ...
    if (edge && noise_idx < noise.size()) { // Add null/bounds check
...
std::vector<std::string> Search::GetVerboseStats(Node* node) const {
  ...
  for (const auto& edge : edges) {
    ...
    print_head(&oss, move.ToString(true),
               MoveToNNIndex(move, 0), edge.GetN(),
...
EdgeAndNode Search::GetBestRootChildWithTemperature(float temperature) const {
  ...
  int chosen_cumulative_idx =
   ...
   if (chosen_cumulative_idx < 0 || chosen_cumulative_idx >= final_eligible_indices.size()) {
...
// ... (SearchWorker Constructor/Destructor remain the same) ...

// ... (SearchWorker::RunTasks implementation remains the same as previous correct version) ...
void SearchWorker::RunTasks(int tid) { /* ... */ }

// ... (SearchWorker::ExecuteOneIteration implementation remains the same) ...
void SearchWorker::ExecuteOneIteration() { /* ... */ }

// ... (SearchWorker::InitializeIteration implementation remains the same) ...
void SearchWorker::InitializeIteration(std::unique_ptr<BackendComputation> computation) { /* ... */ }

// ... (SearchWorker::GatherMinibatch implementation remains the same as previous correct version) ...
void SearchWorker::GatherMinibatch() { /* ... */ }

// ... (SearchWorker::ProcessPickedTask implementation remains the same as previous correct version) ...
void SearchWorker::ProcessPickedTask(int start_idx, int end_idx, TaskWorkspace* workspace) { /* ... */ }

// ... (SearchWorker::ResetTasks implementation remains the same) ...
void SearchWorker::ResetTasks() { /* ... */ }

// ... (SearchWorker::WaitForTasks implementation remains the same) ...
int SearchWorker::WaitForTasks() { return 0; /* ... */ } // Placeholder return

// ... (SearchWorker::PickNodesToExtend implementation remains the same as previous correct version) ...
void SearchWorker::PickNodesToExtend(int collision_limit) { /* ... */ }

// ... (SearchWorker::EnsureNodeTwoFoldCorrectForDepth implementation remains the same) ...
void SearchWorker::EnsureNodeTwoFoldCorrectForDepth(Node* child_node, int depth) { /* ... */ }

// --- Modified PickNodesToExtendTask ---
                 ...
                 for (int i = 0; i <= vtp_last_filled.back(); i++) {
                      if (i < 256) {
                          ...
                          if (task_workers_ > 0 && child_limit > 0 &&
                              ...
                              child_limit < ((collision_limit - passed_off - completed_visits) * 2 / 3) && // Ensure denominator > 0
...
void SearchWorker::FetchSingleNodeResult(NodeToProcess* node_to_process) {
  ...
  if(node->HasChildren()) {
      ...
      if (policy_size != node->GetNumEdges() && policy_size != 0) {
           LOGFILE << "Warning: Policy size mismatch (" << policy_size << ") vs num edges (" << node->GetNumEdges() << ")";
      ...
      LOGFILE << "Warning: Received policy for node with no children.";
  }
}
...
void SearchWorker::DoBackupUpdateSingleNode(
  ...
  for (Node *n = node, *p = nullptr; ; n = p) {
    ...
    update_parent_bounds =
        old_update_parent_bounds && p != search_->root_node_ && !parent_is_final &&
...
bool SearchWorker::MaybeSetBounds(Node* p, float m, int* n_to_fix,
                                  Value* v_delta, float* d_delta, // Use Value* for v_delta
    ...
    GameResult original_lower = p->GetBounds().first;
    GameResult original_upper = p->GetBounds().second;
    ...
    if (p->HasChildren()) {
         ...
         for (const auto& edge : p->Edges()) {
             ...
             if (child && child->is_known_win.load(std::memory_order_relaxed)){
                 ...
                 child_lower = child->GetBounds().first;
                 child_upper = child->GetBounds().second;
             } else if (child){
        ...
        // Final bound determination based on aggregated results
         ...
         } else if (all_children_win_or_draw) { // Can avoid loss, can potentially win, all moves are win/draw
