...
// Pre-include necessary headers defining types used in search.h/node.h
#include "chess/chess.h"         // Defines Value, Move, GameResult, Eval, kValueMate, FlipGameResult etc.
#include "chess/position.h"     // Defines PositionHash, ChessBoard, PositionHistory etc.
#include "config.h"              // Defines LOGFILE
#include "neural/network.h"      // Defines Backend, BackendAttributes, EvalResult, EvalPosition, EvalResultPtr, BackendComputation etc.
#include "search/stats.h"        // Defines IterationStats
#include "search/search_stopper.h" // Defines SearchStopper, StoppersHints
#include "syzygy/syzygy.h"       // Defines SyzygyTablebase, WDLScore, ProbeState etc.
#include "tree/nodetree.h"       // Defines NodeTree
#include "uci/uciresponder.h"    // Defines UciResponder, ThinkingInfo, BestMoveInfo
#include "utils/exception.h"     // Defines Exception
#include "utils/optionsdict.h"   // Defines OptionsDict

// Now include the header for this file
...
// Other required lc0 headers (already included via pre-includes or search.h, but listed for clarity)
#include "neural/encoder.h" // Defines MoveToNNIndex
#include "search/classic/node.h" // Included by search.h
...
// within this file itself. Ensure the file exists in src/utils/ and is correctly
// included in src/trainingdata/trainingdata.h (likely as #include "utils/bitops.h").

namespace lczero {
namespace classic {

// Constants moved inside the namespace in previous step, ensure they are defined after Value is known
...
class MEvaluator {
 public:
  ...
  void SetParent(const Node* parent) {
    //assert(parent); // Allow null parent? The caller MEvaluator(params_, node) passes node which might be null? No, MEvaluator constructor handles null parent. SetParent expects non-null.
...
SearchWorker::~SearchWorker() {
    ...
    if (picker_thread_.joinable()) {
        // Signal the picker thread to stop if it's still waiting
...
void SearchWorker::RunBlocking() {
  ...
  RunTasks(0);
  if (picker_thread_.joinable()) {
      // Ensure the picker thread is notified to exit if it's waiting
      task_added_.notify_all();
      picker_thread_.join();
  }
}
...
void SearchWorker::RunTasks(int tid) {
  //const int collision_limit = params_.GetMaxCollisionEvents(); // Unused
  while (!search_->stop_.load(std::memory_order_acquire)) {
    if (tid == 0) {
      ...
      // Check stop condition *after* waiting but *before* processing
      if (search_->stop_.load(std::memory_order_acquire)) break;
      if (count == 0) {
        // No tasks, just loop and check stop condition again
        // Avoid sleeping if we might need to exit soon
        continue;
      } else {
   ...
   // If this is the picker thread (tid=1), ensure any remaining tasks are cleared upon exit
   if (tid == 1) {
       ResetTasks();
   }
}
...
void SearchWorker::ProcessPickedTask(int start_idx, int end_idx, TaskWorkspace* workspace) {
  ...
  {
    Mutex::Lock lock(picking_tasks_mutex_);
    // Ensure end_idx does not exceed current size
    end_idx = std::min(end_idx, static_cast<int>(picking_tasks_.size()));
    // Ensure start_idx is valid
    start_idx = std::min(start_idx, end_idx);

    if (start_idx < end_idx) {
        tasks.reserve(end_idx - start_idx);
        // Move elements carefully
        std::move(picking_tasks_.begin() + start_idx, picking_tasks_.begin() + end_idx, std::back_inserter(tasks));
        picking_tasks_.erase(picking_tasks_.begin() + start_idx, picking_tasks_.begin() + end_idx);
    }
  }
  for (auto& task : tasks) {
...
int SearchWorker::WaitForTasks() {
    ...
    if (search_->stop_.load(std::memory_order_acquire)) {
        return 0; // Return 0 if search stopped
    }
...
void SearchWorker::EnsureNodeTwoFoldCorrectForDepth(Node* child_node, int depth) {
    ...
    if (!child_node->IsTerminal() && !child_node->is_known_win.load(std::memory_order_relaxed) && !child_node->is_known_loss.load(std::memory_order_relaxed)) {
        ...
        PositionHistory temp_history = search_->GetPositionHistoryAtNode(child_node);
         // Check history validity before accessing Last()
         if (temp_history.GetLength() > 0 && temp_history.Last().GetRepetitions() >= 1) { // Check for 2-fold (>=1 repetition)
    ...
    while (current_path.size() > 0) {
        ...
        if (current_path.back() == -1) {
            ...
            if (current_path.size() > 1) {
                // Bounds check for visits_to_perform and the index
                size_t parent_path_idx = current_path.size() - 2;
                int parent_edge_idx = current_path[parent_path_idx];
                if (parent_path_idx < visits_to_perform.size() && parent_edge_idx >= 0 && parent_edge_idx < 256){
                    cur_limit = (*visits_to_perform[parent_path_idx])[parent_edge_idx];
                } else {
                    // Fallback or log error if indices are invalid
                    cur_limit = collision_limit - completed_visits - passed_off; // Safe fallback
                     // Optional logging for debugging unexpected state:
                    // if (parent_path_idx >= visits_to_perform.size()) LOGFILE << "Warning: visits_to_perform out of bounds";
                    // if (parent_edge_idx < 0 || parent_edge_idx >= 256) LOGFILE << "Warning: parent edge index out of bounds";
                }
            }
            if (cur_limit <= 0) {
                ...
                if (!visits_to_perform.empty()) {
                    if (visits_to_perform.back()) { // Ensure unique_ptr is valid
            ...
            if (vtp_buffer.size() > 0) {
                 if (vtp_buffer.back()) { // Ensure unique_ptr is valid before move
                    visits_to_perform.push_back(std::move(vtp_buffer.back()));
                    vtp_buffer.pop_back();
                 } else {
                     // Handle error: null unique_ptr in buffer
                     visits_to_perform.push_back(std::make_unique<std::array<int, 256>>());
                 }
            } else {
                visits_to_perform.push_back(std::make_unique<std::array<int, 256>>());
            }
             // Ensure the newly added unique_ptr is valid
             if (!visits_to_perform.back()) {
                 visits_to_perform.back() = std::make_unique<std::array<int, 256>>();
             }
            ...
            while (cur_limit > 0) {
                 // Check stop condition inside inner loop too
                 if (search_->stop_.load(std::memory_order_acquire)) break;
                     ...
                     for (int idx = 0; idx < max_needed; ++idx) {
                         ...
                         if (is_root_node) {
                             ...
                             { // Lock only for reading current_best_edge_ N
                              ...
                              // Make sure best_node_n is up-to-date before comparison
                             best_node_n = current_best_n; // Update local copy
                             if (cur_iters[idx] != search_->current_best_edge_ &&
                                 latest_time_manager_hints_.GetEstimatedRemainingPlayouts() <
                                     best_node_n - cur_iters[idx].GetN()) { // Use updated best_node_n
                               continue;
                             }
                         ...
                         if (score > best) {
                             ...
                             best = score;
                             best_idx = idx;
                             best_without_u = overridden ? kValueKnownLoss : util; // Use Value constant
                ...
                if (visits_to_perform.empty() || vtp_last_filled.empty() || !visits_to_perform.back()) {
                     assert(false && "visits_to_perform or vtp_last_filled became empty/null unexpectedly");
                     break;
                }
                ...
                if (child_node && child_node->TryStartScoreUpdate()) {
                     ...
                     // Only increment NInFlight for the *remaining* visits after the first one used for the visit/NN eval
                ...
                if (child_node && decremented &&
                    (child_node->GetN() == 0 || child_node->IsTerminal() || (proven_state_enabled && (child_node->is_known_win.load(std::memory_order_relaxed) || child_node->is_known_loss.load(std::memory_order_relaxed))) )) {
                     // If we successfully started a score update (decremented=true) and the node is a leaf/terminal/proven,
                     // then this first visit should be added to the minibatch for NN eval or collision processing.
                     if (best_idx >= 0 && best_idx < 256) {
                         // This '1' corresponds to the initial visit used for TryStartScoreUpdate
                         // The remaining 'new_visits' were handled by IncrementNInFlight above for non-terminals
                         // We subtracted 1 from new_visits if decremented=true, so the accounting seems okay.
                         // However, we need to remove this *single* visit count from the visits_to_perform array
                         // because it's now being handled by the NodeToProcess::Visit entry.
                   ...
                   // Ensure the node added for visit has eval allocated
                   if (!receiver->back().eval) {
                       receiver->back().eval = std::make_shared<EvalResult>();
                   }
                   // Mark that this node will need NN evaluation (unless terminal/proven)
                   receiver->back().nn_queried = !child_node->IsTerminal() &&
                                                  !(proven_state_enabled &&
                                                    (child_node->is_known_win.load(std::memory_order_relaxed) ||
                                                     child_node->is_known_loss.load(std::memory_order_relaxed)));
             ...
             // Check stop condition before splitting tasks
             if (search_->stop_.load(std::memory_order_acquire)) break;

            is_root_node = false;
             // Split logic
             if (!visits_to_perform.empty() && !vtp_last_filled.empty() && vtp_last_filled.back() >= 0 && node && node->HasChildren() && visits_to_perform.back()) {
                 for (int i = 0; i <= vtp_last_filled.back(); i++) {
                      if (i < 256) {
                          int child_limit = (*visits_to_perform.back())[i];
                           // Adjust split condition: Ensure collision_limit is positive before division
                           int remaining_limit = collision_limit - passed_off - completed_visits;
                           if (remaining_limit <= 0) continue; // Avoid division by zero or negative

                          if (task_workers_ > 0 && child_limit > 0 &&
                              child_limit > params_.GetMinimumWorkSizeForPicking() &&
                              child_limit < (remaining_limit * 2 / 3) &&
                              child_limit + passed_off + completed_visits < collision_limit - params_.GetMinimumRemainingWorkSizeForPicking()) {
                               ...
                               {
                                   Mutex::Lock lock(picking_tasks_mutex_);
                                    // Check stop condition again before adding task
                                    if (search_->stop_.load(std::memory_order_acquire)) break;
                                   if (picking_tasks_.size() < MAX_TASKS) {
                                       if(child_iter) {
                                           moves_to_path.push_back(child_iter.GetMove());
                                           picking_tasks_.emplace_back(
                                               child_node, static_cast<uint16_t>(current_path.size() + base_depth + 1), // Increment depth for child
        ...
        // Check stop condition before selecting next child
        if (search_->stop_.load(std::memory_order_acquire)) break;

        // Select next child logic
        int min_idx = current_path.back();
        bool found_child = false;
        if (!visits_to_perform.empty() && !vtp_last_filled.empty() && vtp_last_filled.back() > min_idx && node && node->HasChildren() && visits_to_perform.back()) {
            int idx = -1;
...
void SearchWorker::ExtendNode(Node* node, int depth,
    ...
    if (!node || !history || history->GetLength() == 0) return; // Add null checks and history check
...
bool SearchWorker::AddNodeToComputation(Node* node) {
    ...
    if (node_history.GetLength() == 0) {
        ...
        LOGFILE << "Warning: Invalid history in AddNodeToComputation for node " << node->GetHash();
        return false;
    }
    ...
    return computation_->AddInput(EvalPosition{node_history.GetPositions(), moves},
                                  EvalResultPtr{}) == // Pass empty EvalResultPtr
...
int SearchWorker::PrefetchIntoCache(Node* node, int budget, bool is_odd_depth) {
  ...
  for (size_t i = 0; i < scores.size(); ++i) {
        ...
        if (i + 1 >= scores.size()) {
             ...
             if (denom > 1e-9f) { // Avoid division by zero/small numbers
                 budget_to_spend =
                  ...
                  // If denominator is near zero, give it full budget? Or 1? Let's give 1.
                 budget_to_spend = std::min(budget, 1);
             }
        }
    } else {
    ...
    if (history_.GetLength() < 512) { // Example limit
        ...
        // Ensure child node exists before recursing
        Node* child_node = edge.GetOrSpawnNode(node);
        const int budget_spent =
            PrefetchIntoCache(child_node, budget_to_spend, !is_odd_depth);
        history_.Pop();
        budget -= budget_spent;
...
void SearchWorker::RunNNComputation() {
  if (computation_->UsedBatchSize() > 0) {
      // Prepare EvalResult pointers for the batch
      for (auto& node_to_process : minibatch_) {
          if (node_to_process.node && node_to_process.nn_queried && node_to_process.eval) {
              // Pass the shared_ptr stored in NodeToProcess to the computation
              computation_->AddEvalResultPtr(node_to_process.node->GetHash(), node_to_process.eval);
          }
      }
...
void SearchWorker::FetchSingleNodeResult(NodeToProcess* node_to_process) {
  ...
  // Ensure eval pointer exists where results should be stored
  if (!node_to_process->eval) {
      LOGFILE << "Error: node_to_process->eval is null in FetchSingleNodeResult for node " << node->GetHash();
      return; // Cannot proceed without eval results container
  }

  if (!node_to_process->nn_queried) {
    // If NN wasn't queried (e.g., terminal/proven node discovered during selection),
    // populate eval with the node's final state.
    node_to_process->eval->q = node->GetWL(); // Use WL for terminal Q
    node_to_process->eval->d = node->GetD();
    node_to_process->eval->m = node->GetM();
    // Policy vector should be empty or ignored in this case.
    node_to_process->eval->p.clear();
    return;
  }

  // If NN *was* queried, the results should already be in node_to_process->eval
  // shared_ptr via RunNNComputation and Backend processing.
  // We just need to apply post-processing like perspective flip and WDL rescale.
  ...
  // Apply policy if node has children (and policy exists)
  if(node->HasChildren()) {
      size_t p_idx = 0;
      ...
      if (policy_size != static_cast<size_t>(node->GetNumEdges()) && policy_size != 0) {
           ...
           // Handle mismatch: zero out policy or proceed cautiously? Let's proceed.
      }
...
void SearchWorker::DoBackupUpdateSingleNode(
  ...
  if (!node_to_process.eval) {
      ...
      // Cancel score update to decrement n_in_flight.
  ...
  for (Node *n = node, *p = nullptr; ; n = p) {
     ...
     if (proven_state_enabled &&
         (n->is_known_win.load(std::memory_order_relaxed) ||
          n->is_known_loss.load(std::memory_order_relaxed))) {
            // Propagate the proven value up the tree
            v = n->is_known_win.load(std::memory_order_relaxed) ? kValueKnownWin : kValueKnownLoss;
            d = 0.0f;
            m = n->GetM(); // Use the M value calculated for this proven node
            // Finalize this node's update (mainly for n_in_flight)
            n->FinalizeScoreUpdate(v, d, m, node_to_process.multivisit);
            // Continue propagation with the proven values
     } else {
    ...
    // Check bounds and set flags on parent *after* parent potentially gets updated by MaybeSetBounds
    // Defer this check until after the MaybeSetBounds call below.
    ...
    if (parent_is_final) {
        ...
        // --- Proven State Check on Parent 'p' (after potential bound update) ---
        if (proven_state_enabled && !parent_is_final) { // Check parent_is_final again in case MaybeSetBounds made it final
             auto [p_lower, p_upper] = p->GetBounds(); // Get potentially updated bounds
             if (p_upper >= kValueKnownWin && !p->is_known_win.load(std::memory_order_relaxed)) {
                 p->is_known_win.store(true, std::memory_order_relaxed);
             }
             if (p_lower <= kValueKnownLoss && !p->is_known_loss.load(std::memory_order_relaxed)) {
    ...
    if (p == search_->root_node_) {
        // Need to check if n *became* terminal/proven *in this update cycle*
        // Check IsTerminal() status *after* the FinalizeScoreUpdate/AdjustForTerminal call on 'n'
        bool n_is_now_final = n->IsTerminal() || (proven_state_enabled && (n->is_known_win.load(std::memory_order_relaxed) || n->is_known_loss.load(std::memory_order_relaxed)));
        bool n_was_final_before_update = n->IsTerminal(true) || (proven_state_enabled && ( n->is_known_win.load(std::memory_order_relaxed) || n->is_known_loss.load(std::memory_order_relaxed))); // How to get previous state? Difficult.

        // Simpler logic: Update root's best edge if the child node ('n') is the one associated with the current best edge
        // and its stats changed, OR if 'n' is NOT the current best edge's node but now has more visits.
         EdgeAndNode current_best = search_->current_best_edge_; // Local copy
         if (current_best.node() == n || // If n *is* the node for the best edge
             (current_best.node() != n && current_best.GetN() <= n->GetN())) { // Or if n might overtake the best edge
           search_->current_best_edge_ =
               search_->GetBestChildNoTemperature(search_->root_node_, 0);
         }
    }
...
bool SearchWorker::MaybeSetBounds(Node* p, float m, int* n_to_fix,
    ...
    if (new_lower != original_lower || new_upper != original_upper) {
         ...
         bounds_changed = true;
        // Proven state flags are now set in the backup loop after this call
    }
...
void SearchWorker::UpdateCounters() {
  ...
  if (!work_done) {
    // Avoid busy-waiting if no work was done (e.g., only collisions processed)
    // This can happen if the tree is fully explored or only collisions remain.
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  }
}
