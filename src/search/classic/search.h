...
class Search {
 ...
 private:
  ...
  Node* root_node_;
  SyzygyTablebase* syzygy_tb_; // <<< NO lczero:: (Assuming accessible)
  const PositionHistory& played_history_; // <<< NO lczero::

  Backend* const backend_; // <<< NO lczero:: (Assuming accessible)
  BackendAttributes backend_attributes_; // <<< NO lczero:: (Assuming accessible)
  ...
  std::unique_ptr<UciResponder> uci_responder_; // <<< NO lczero:: (Assuming accessible)
